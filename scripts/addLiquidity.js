...
const addLiquidity = async () => {
  ...
  const ETH_ADDRESS = "0x4f7AB399837080229798574866700C066CBc3dc2"; 
  const USDT_ADDRESS = "0xD0dB1ECd4cdfBaeF6707C2065529091f93BD9382"; 
  ...
  console.log("Checking if contracts are properly initialized...");
  try {
    await routerContract.factory();
    console.log("Router contract initialized successfully");
  } catch (error) {
    console.error("Error accessing router contract:", error.message);
    throw new Error("Router contract initialization failed");
  }

  const ethAmount = ethers.parseUnits("10", 18);
  const usdtAmount = ethers.parseUnits("1000", 6); 
  const minAmountETH = ethers.parseUnits("9", 18); // Set minimum amount to 90% of desired amount
  const minAmountUSDT = ethers.parseUnits("900", 6); // Set minimum amount to 90% of desired amount
  ...
  console.log("Balances and allowances are sufficient.");
  console.log('ETH Allowance:', (await ethContract.allowance(signer.address, routerAddress)).toString());
  console.log('USDT Allowance:', (await usdtContract.allowance(signer.address, routerAddress)).toString());

  console.log("Adding liquidity...");
  const PAIR_ADDRESS = '0x4Ddc20683d1833D80f13a0857501913b24987Cb9';
  try {
